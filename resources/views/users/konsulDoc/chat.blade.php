...
@section('container')
        ...
        <div class="row">
            <div class="card">
                    ...
                    @foreach($messages->sortBy('created_at') as $message)
                        <div class="message d-flex justify-content-{{ $message->sender_id == Auth::id() ? 'end' : 'start' }}">
                            <div class="alert {{ $message->sender_id == Auth::id() ? 'alert-primary' : 'alert-secondary' }}">
                                <strong>{{ $message->sender_id == Auth::id() ? 'me' : 'dr ' . $doctor->where('id', $message->sender_id)->first()->name }}</strong>
                                <br>
                                {{ $message->content }}
                            </div>
                        </div>
                ...
                <div class="card-footer">
                    ...
                    <form id="chatForm" action="{{ url('users/konsulDoc/send/' . session('doctor_id')) }}" method="post">
                        @csrf
                        <input type="hidden" name="receiverId" value="{{ session('doctor_id') }}">
    ...
    <script>
        ...
        $('#sendButton').on('click', function () {
            ...
            $.ajax({
                url: '{{ url("users/konsulDoc/send/" . session("doctor_id")) }}',
                type: 'POST',
                data: {
                    _token: $('input[name=_token]').val(),
                    content: messageContent,
                    receiverId: {{ session('doctor_id') }}
                },
